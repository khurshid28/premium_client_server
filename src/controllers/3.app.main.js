...
class App {
  ...
  async  arizaAll(req, res, next) {
    ...
    try {
      zayavkalar = await new Promise(function (resolve, reject) {
        db.query(
          `SELECT * FROM Ariza where client_id=${req.user.id}`,
          function (err, results, fields) {
            if (err) {
              resolve(null);
              return null;
  ...
  async createAriza(req, res, next) {
    ...
    try {
      ...
      let createdzayavka = await new Promise(function (resolve, reject) {
        db.query(
          `SELECT * from Ariza WHERE client_id=${req.body.id}`,
          function (err, results, fields) {
            if (err) {
              resolve(null);
              return null;
  ...
  async PostScoring(req, res, next) {
    ...
    try {
      ...
      let zayavka = await new Promise(function (resolve, reject) {
        db.query(
          `SELECT * from Ariza WHERE id=${req.body.id}`,
          function (err, results, fields) {
            if (err) {
              resolve(null);
              return null;
...
function createArizaFunc(data) {
  let {
    ...
    return `insert into  Ariza  (fullName,phoneNumber,phoneNumber2,cardNumber,passport_date,passport_by,address,region_id,cardId,client_id,passport) values('${fullname}',${phoneNumber},${phoneNumber2},${cardNumber},'${passport_date}','${passport_by}','${address}',${region_id},'${cardId}',${id},'${passport}') ; `;
